...
*Starvation occurs when a thread is not able to access a shared resource among multiple thread due to there being a lot of threads competing for the same shared resources. Starvation may also occur when a single thread locks the shared resource and forgets to unlock it therefore blocking all the other threads from using it.
In our case, since there are 54 artists and a single canvas to paint on, the artists are competing for access to pain on the canvas. When an artist tries to get a hold of the canvas to paint, if it’s already being used by another artist then he would not able to paint. To fix this we can implement a mechanism that keeps the threads on waitlist, based on the waiting time the threads may be given access to the shared resources.*
