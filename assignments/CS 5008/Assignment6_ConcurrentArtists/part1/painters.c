...
void* paint(void* args){
    ...
    for(int i =0; i < 5000; ++i){
        ...
        if(pthread_mutex_trylock(&canvas[painter->x][painter->y].lock) == 0){
            ...
            else if(canvas[painter->x][painter->y].r == painter->r && canvas[painter->x][painter->y].g == painter->g && canvas[painter->x][painter->y].b == painter->b){
                pthread_mutex_unlock(&canvas[painter->x][painter->y].lock);
            }
...
static int colors[256][256][256] = {0}; // // create a matrix to store the color used by artists
static int positions[256][256] = {0}; // create a matrix to store the positions occupied by artists

void getPosition(int m[256][256], int c[256][256][256],  int* x, int* y, int* r, int* g, int* b) {
    srand(time(NULL));
    *x = rand() % 256;
    *y = rand() % 256;
    while(1){
...
int main(){
    ...
    artist_t** moreArtists = malloc (sizeof(artist_t*) * 50); // Create an array that holds out artists
    
    positions[Michaelangelo->x][Michaelangelo->y] = 1; // mark the postion of the expert artists as occupied
    positions[Donatello->x][Donatello->y] = 1;
    positions[Raphael->x][Raphael->y] = 1;
    positions[Leonardo->x][Leonardo->y] = 1;
    
    
    colors[Michaelangelo->r][Michaelangelo->g][Michaelangelo->b] = 1; // mark the color of the expert artists as used
    colors[Donatello->r][Donatello->g][Donatello->b] = 1;
    colors[Raphael->r][Raphael->g][Raphael->b] = 1;
    colors[Leonardo->r][Leonardo->g][Leonardo->b] = 1;
    
    int x,y,r,g,b;
    // Fill in the artist attributes
    for(int i =0; i < rookieArtists; ++i){
        moreArtists[i] = malloc(sizeof(artist_t));
        getPosition(positions,colors,&x,&y,&r,&g,&b); // get the position for the artist
        positions[x][y] = 1; // mark the positon of the artist as occupied
        colors[r][g][b] = 1; // mark the color of the artist as occupied
        moreArtists[i]->r = r;
        moreArtists[i]->g = g;
        moreArtists[i]->b = b;
        moreArtists[i]->x = x;
    ...
    //Free the array holding the artists
    free(moreArtists);
