...
queue_t* create_queue(unsigned int _capacity){
    	queue_t* myQueue = (queue_t*) malloc(sizeof(queue_t));; // allocate space for queue on the heap
    	myQueue->data = (int*) malloc(sizeof(int) * _capacity); // allocate space for the array on the heap
...
int queue_empty(queue_t* q){
    	if(q->front == q->back) // if queue front and back are equal that means that the queue is either recently emptied or its a new queue
        	return 1;
    	else
        	return 0;
}
...
int queue_full(queue_t* q){
    	if(q->size == q->capacity) // if the current size of the queue is same as the queue capacity then the queue is full
        	return 1;
    	else
        	return 0;
}
...
int queue_dequeue(queue_t *q){
	...
	int dequeueElement = q->data[q->back];
    	q->data[q->back] = 0;
