...
Stack is a useful data structure when you want to process data in a specific order. Stack follows LIFO(last in first out), so the last element to be added to the stack is removed first. This is a very useful data structure in situations where you always need to have access to the newest element that was inserted into the stack. But if you need access to elements in random positions, stack would not be a good choice since you’ll have to remove the top element till you reach the desired element, which might not be a good idea.  

Stacks are a widely used data structure, they can be used to evaluate arithmetic expressions, they are also used in programming languages to manage function calls. When a program with function calls in main is run, the functions are pushed onto the call stack and when the function returns the values it is popped from the call stack. It can also be used to implement undo/redo operations in applications.
