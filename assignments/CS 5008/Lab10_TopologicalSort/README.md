...
1. *If there are N nodes in a graph then we need a 2D array of N rows and N columns, so we will need N*N space. So the space complexity is O(N^2).*
2. *Since we are establishing the edges between nodes, and each node may have an in or an out edge with another node, we need to have rows to be equal to columns. So if we want to be able to model relationships between every node in a graph rows!=columns doesn’t make sense.*
...
1. *Topological sort sorts a directed acyclic graph in the ascending order of number of in-degrees. So running topological sort is not applicable to an undirected graph.*
2. *We cannot run Topological sort for a graph with a cycle.  Topological sort is mean for directed acyclic graph, running it for a cyclic graph will give an error saying “input contains a loop”. *
3. *To detect cycles in a directed graph, we can use DFS and keep track of visited nodes and the recursion stack. If we encounter a node that is already in the recursion stack, then a cycle exists*
